...
def create_docs_router(
        config_or_path: Union[DocsConfig, str, Path]) -> APIRouter:
    ...
    async def get_document_metadata(path: str):
        ...
        return {
            ...
            "prev": {
                "title": prev_node.metadata.title,
                "path": prev_node.path
            } if prev_node else None,
            "next": {
                "title": next_node.metadata.title,
                "path": next_node.path
    ...
    async def serve_document(request: Request, path: str = ""):
        ...
        # Compute base prefix for building absolute links
        # within the mounted router
        mount_prefix = (request.url.path[:-len(path)]
                        if path else request.url.path)
